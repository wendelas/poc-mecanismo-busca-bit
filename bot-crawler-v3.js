...
async function updateInternalLinksWithDepth(urls, baseDomain, depth, maxDepth) {
    ...
    try {
        ...
        for (const url of urls) {
            ...
            await updateInternalLinksWithDepth(linkData.internalLinks, baseDomain, depth + 1, maxDepth);
        }
...
// Função para gerar o sitemap com níveis de profundidade
async function generateSitemap() {
    ...
    try {
        ...
        for (const site of sites) {
            ...
            for (const link of site.internalLinks) {
                sitemap.push({
                    url: link,
                    depth: (site.depth || 0) + 1
                });
            }
        }

        // Remove URLs duplicadas
        const uniqueSitemap = Array.from(new Set(sitemap.map(item => item.url)))
            .map(url => sitemap.find(item => item.url === url));

        await sitemapCollection.insertMany(uniqueSitemap);
        console.log('Sitemap gerado e salvo na coleção sitemap.');
    } catch (error) {
...
app.get('/generate-sitemap', async (req, res) => {
    try {
        ...
        res.send('Sitemap gerado e salvo.');
    } catch (error) {
        res.status(500).send({ error: error.message });
    }
});
